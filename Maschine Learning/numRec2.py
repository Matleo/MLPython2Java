...
def determinNumber(tArray,i):
    ...
    guessed= sess.run(tf.argmax(y_conv, 1), feed_dict={x:output,keep_prob: 1})
    print(guessed)
    """guessedIndex= sess.run(tf.argmax(y_conv,1), feed_dict={x:output})
...
with tf.Session() as sess:
    ...
    for i in range(0,10):
        file = tf.read_file('./Own_dat/Computer-'+str(i)+'.png')
